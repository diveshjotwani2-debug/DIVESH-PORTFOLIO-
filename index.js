...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --------------------------------------------------------------------------
    // B. Custom Sword Cursor Background Processor
    // Extracts the black background of the sword PNG in real-time to guarantee
    // clean transparency across different browsers.
    // --------------------------------------------------------------------------
    const processSwordImage = () => {
        const img = new Image();
        img.src = 'realistic-sword.png';
        ...
        img.onload = () => {
            try {
                ...
                // Make black or near-black background pixels fully transparent
                ...
                if (cursorEl) {
                    cursorEl.style.backgroundImage = `url('${transparentDataURL}')`;
                }
            } catch (err) {
                console.warn("Local canvas processor bypassed due to security context. Standard PNG active.", err);
            }
        };
    };

    // --------------------------------------------------------------------------
    // C. Custom Sword Cursor Physics & Hover States
    // --------------------------------------------------------------------------
    ...
    const initCursor = () => {
        window.addEventListener("mousemove", (e) => {
            ...
            if (!cursorActive) {
                ...
                if (cursorGlow) cursorGlow.style.opacity = "0.35";
            }
        });
        ...
        // Trigger sword cursor rotation & scaling on hover
    ...
    const frameLoop = () => {
        ...
        // 2. LERP Sword Cursor position (aligns handle center to pointer)
        if (cursorActive && cursorEl && cursorGlow) {
            curX += (mouseX - curX) * cursorEase;
            curY += (mouseY - curY) * cursorEase;
            
            cursorEl.style.transform = `translate3d(${curX - 22}px, ${curY}px, 0) rotate(var(--cursor-rotate, 0deg)) scale(var(--cursor-scale, 1))`;
